...
#include "STM32_F103C6_ADC_Driver.h"
...
SPI_Config_t* Global_ADC_Config[2]  	= {NULL,NULL} ;
SPI_Config_t Global_ADC_Config1 ;		//taking copy of config as if user  takes an object from config struct into function otherwise main
SPI_Config_t Global_ADC_Config2 ;       // its stack will close deleting the config object in stack of this function
 ...
 * @param [in] 		- ADCx: where x can be (1..3 depending on device used)
 * @param [in] 		- ADC_Config: All ADC Configuration
 * @retval 			- none
 * Note				- none
 */
void MCAL_SPI_Init (ADC_TypeDef *ADCx, ADC_Config_t* ADC_Config)
{
	...
	if (ADCx == ADC1)
	{
		Global_ADC_Config1 = *ADC_Config ;
		Global_ADC_Config[ADC1_INDEX] =  &Global_ADC_Config1 ;
		RCC_ADC1_CLK_EN();

	}else if (ADCx == ADC2)
	{
		Global_ADC_Config2 = *ADC_Config ;
		Global_ADC_Config[ADC2_INDEX] =  &Global_ADC_Config2 ;
		RCC_ADC2_CLK_EN();
